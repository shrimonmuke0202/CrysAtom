...
parser.add_argument('--atomvector_path', default='', type=str, help='Pretrained Vector Path')
...
def main():
    ...
    # load data
    dataset = CIFData(data_path,args.atomvector_path)
    ...
    if datasize < 2000:
        sample_data_list = [dataset[i] for i in tqdm(range(datasize))]
    else:
        sample_data_list = [dataset[i] for i in tqdm(sample(range(datasize), 2000))]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        t = time.time()
        # train for one epoch
        train_loss_p, train_mae_error = train(train_loader, model, criterion, optimizer, epoch, normalizer,args.alpha)
        ...
        print('Epoch: {:d}'.format((epoch + 1)),
              ...
              'Test MAE: {:.3f}'.format(test_mae_error),
              'Best Test MAE: {:.3f}'.format(best_mae_error),
        ...
        if args.is_log:
            out.writelines('Epoch: {:d}'.format((epoch + 1)) +
                           ...
                           '\t Train MAE: {:.3f}'.format(train_mae_error) +
                           '\t Val MAE: {:.3f}'.format(val_mae_error) +
                           '\t Test MAE: {:.3f}'.format(best_mae_error) +
                           '\t Best Test MAE: {:.3f}'.format(best_mae_error) + 
                           '\t time: {:.4f} min'.format((time.time() - t) / 60))
            out.writelines("\n")
...
def train(train_loader, model, criterion, optimizer, epoch, normalizer,alpha):
    # losses = AverageMeter()
    # print("Inside Train")
    ...
    return losses_p.avg,mae_errors.avg
