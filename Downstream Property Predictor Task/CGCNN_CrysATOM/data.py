...
class PKLData(Dataset):
    ...
    def __init__(self, root_dir):
        ...
        self.atom_init_dict = json.load(open(atom_init_file))
        self.rev = {tuple(self.atom_init_dict[key]):key for key in self.atom_init_dict}
        self.atom_vector = pkl.load(open("dense_vector/CrysAtom_vector.pkl","rb"))
    ...
    def __getitem__(self, idx):
        ...
        #     sentences[0][0], sentences[0][1], sentences[0][2], sentences[1], sentences[2]
        
        atom_fea, nbr_fea, nbr_fea_idx, target,spacegroup = \
            sentences[0], sentences[1], sentences[2], sentences[3], sentences[4]
        
        
        # atom_feat = np.asarray(atom_fea.todense(),dtype='i4')
        # atom_feat = np.vstack([self.atom_vector[int(self.rev[tuple(i)])] for i in atom_feat.tolist()])
...
class CIFData(Dataset):
    def __init__(self, root_dir,atom_vector_path, max_num_nbr=12, radius=8, dmin=0, step=0.2,random_seed=123):
        self.root_dir = root_dir
        ...
        self.ari = pkl.load(open(atom_vector_path,'rb'))
    ...
    def __getitem__(self, idx):
        ...
        # Atom feature
        atom_fea = np.vstack([self.ari[crystal[i].specie.number] for i in range(len(crystal))])
